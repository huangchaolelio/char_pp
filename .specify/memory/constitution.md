...
递增理由: MINOR — 新增原则 X「业务流程对齐」，将 docs/business-workflow.md
  确立为系统设计与优化的权威业务参考，明确要求：
  (1) 每个 Feature 的 spec.md 必须声明其所属业务阶段（训练 / 建标 / 诊断）
      与步骤（对齐 business-workflow.md § 3–§ 5 八步骤清单）；
  (2) 新增/改动的 Celery 队列、状态机、错误码前缀、评分公式、单 active
      等章程级约束必须与 business-workflow.md 双向同步；
  (3) 设计优化（时效性 / 准确性 / 成本）必须命中 business-workflow.md § 9
      的三种杠杆定位，禁止在业务流程外引入旁路路径。
  属于实质性扩展（新强制约束、新必需产物、新验证维度），不删除任何既有
  原则、不改变原则 I–IX 语义 → MINOR 递增。

修改的原则:
  - 无既有原则被修改

新增条款:
  - 原则 X「业务流程对齐」：docs/business-workflow.md 为业务流程权威参考，
    Feature 规范、队列/状态机/错误码调整、优化工作均须显式对齐
  - 开发工作流「章程检查」新增第 (e) 项业务流程对齐验证要点

删除的部分:
  - 无

模板同步状态:
  ✅ .specify/templates/plan-template.md — 章程合规验证新增原则 X 条目，
     追加「业务流程对齐验证要点」独立小节
  ✅ .specify/templates/spec-template.md — 功能需求节新增「业务阶段映射」
     必需段落，强制声明 phase / step 与 DoD 交叉引用
  ✅ .specify/templates/tasks-template.md — 无需修改（原则 X 通过 spec/plan
     侧强制即可，任务分类不必新增冗余节）
  ...
  ✅ .codebuddy/rules/api.md — 无需修改（原则 X 聚焦业务流程，与 API 规范正交）
  ✅ .codebuddy/skills/refresh-docs/SKILL.md — 已于同版本周期内纳入
     business-workflow.md 的刷新清单（3 份核心文档并列）
  ✅ docs/business-workflow.md — 本身作为权威参考，无需反向修改
...
```
{
  ...
  "error": {
    ...
    "details": { ... } | null   // 可选结构化上下文（如 resource_id、field、successor）
  }
}
```
...
- 已下线接口 → 404 + `ENDPOINT_RETIRED`（见下「已下线接口台账」）
...
**已下线接口台账**: 接口下线 MUST 保留哨兵路由（方法+路径不变），其处理器统一抛
`AppException(ENDPOINT_RETIRED, details={"successor": "/api/v1/...", "migration_note": "..."})`；
禁止直接物理删除路由（会与 FastAPI 默认 404 `NOT_FOUND` 混淆）。下线台账 MUST 双份维护:
代码侧 `src/api/routers/_retired.py::RETIREMENT_LEDGER` + 文档侧 `specs/<feature>/contracts/retirement-ledger.md`；
台账条目只可追加，不可删除。
...
**下线接口保留 ENDPOINT_RETIRED 哨兵路由**）；(e) 功能满足原则 X（**spec.md 含
...
**版本**: 1.5.0 | **批准日期**: 2026-04-17 | **最后修订**: 2026-04-30
