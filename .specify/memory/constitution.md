...
递增理由: MINOR — 对原则 IX 的两条关键子条款进行实质性扩展与重定义，以支撑
  Feature-017「API 接口统一规范化」的统一响应信封方案：
  (1) 响应体格式从"单实体裸返回 / {data,total}"升级为"顶层 success 布尔位 + data + meta
      互斥信封"，使成功/错误响应形态一致、可由客户端 SDK 机械判别；
  (2) 错误响应映射从"HTTPException + detail"升级为"全局异常处理器 + ErrorBody(code,
      message, details)"，并引入集中化 ErrorCode 枚举 + 单一 HTTP 状态映射表。
  同时新增「已下线接口台账」(RetirementLedger) 作为原则 IX 的配套强制要求。
  属于实质性扩展（新的强制约束、新的必需产物），不删除任何原有原则 → MINOR 递增。

修改的原则:
  - IX. API接口规范统一 — 重写「响应体格式」「错误响应映射」两段；新增「已下线接口台账」段

新增条款:
  - 原则 IX「已下线接口台账」：接口下线必须保留哨兵路由返回 ENDPOINT_RETIRED
  - 附加约束中的「错误码集中化」：要求 src/api/errors.py 为单一事实来源

删除的部分:
  - 无（原有条款仅是被改写而非删除）

模板同步状态:
  ✅ .specify/templates/plan-template.md — 已同步「章程检查」中的响应体与错误响应验证要点
  ✅ .specify/templates/spec-template.md — 无需修改（接口契约仍由 contracts/ 承载）
  ✅ .specify/templates/tasks-template.md — 无需修改（TDD 合约测试任务已覆盖新格式）
  ...
  ✅ .codebuddy/rules/api.md — 项目规则1「API 设计规范」已按 v1.4.0 新规全面重写，
    覆盖统一信封、AppException + 集中化 ErrorCode、ENDPOINT_RETIRED 哨兵路由、
    命名一致性、分层职责、合约测试前置，保留路由清单收敛为 Feature-017 后的 8 个
...
**下线接口保留 ENDPOINT_RETIRED 哨兵路由**）。任何违规 MUST 在继续之前记录并获批。
...
**版本**: 1.4.0 | **批准日期**: 2026-04-17 | **最后修订**: 2026-04-27
