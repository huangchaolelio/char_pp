...
递增理由: PATCH — 对 v1.3.0 新增的"API接口规范统一"原则进行措辞精化与细节补充：
  明确路由分层职责（路由层只做参数校验+响应组装，业务逻辑归 services/）、
  细化错误响应映射（含 `HTTPException` 转换规则）、补充分页响应体严格格式、
  明确新增路由必须先建合约测试。属于对现有原则的澄清，无新原则，无不兼容治理变更。

修改的原则:
  - IX. API接口规范统一 — 细化并强化具体条款（路由分层、错误映射、合约测试前置）
...
模板同步状态:
  ✅ .specify/templates/plan-template.md — 章程检查已包含原则 IX 验证要点（v1.3.0 同步）
  ✅ .specify/templates/spec-template.md — 无需修改（API 契约定义已由 contracts/ 目录承载）
  ✅ .specify/templates/tasks-template.md — 无需修改（合约测试任务已覆盖 API 场景）
...
**响应体格式**: 接口响应体 MUST 统一包装为以下两种格式之一——
(a) 单实体或非分页场景：直接返回实体对象本身（如 `{"id": 1, "name": "..."}`）；
(b) 分页或列表场景：`{"data": [...], "total": N}` 格式，`total` 为符合条件的全量记录数。
禁止直接返回裸数组（如 `[{...}, {...}]`）或在顶层包装 `code/message/result` 等冗余字段。
...
**错误响应映射**: 路由层 MUST 统一捕获服务层异常并转换为 `HTTPException`，映射规则如下：
`ValueError` → 400、`NotFoundException` → 404、其他未预期异常 → 500（且 MUST 记录
`logging.exception` 日志）；禁止将服务层异常直接穿透到客户端，也禁止在路由层自行判断
业务错误。错误响应体 MUST 包含可读的 `detail` 字段。
...
满足原则 IX（统一版本前缀、资源化路由、标准分页、响应体包装、分层职责、错误映射、
合约测试前置）。任何违规 MUST 在继续之前记录并获批。
...
**版本**: 1.3.1 | **批准日期**: 2026-04-17 | **最后修订**: 2026-04-27
