...
正确派生：``(CONTENT_PREP, curate_segments)``（Feature-022 迁入 CONTENT_PREP）。
同步覆盖 ``_assign_phase_step`` ``before_insert`` 钩子链路。
"""
...
def test_video_curation_derives_to_training_curate_segments() -> None:
    """video_curation 派生到 (CONTENT_PREP, curate_segments)。"""
...
def test_video_curation_in_phase_step_matrix_and_phase_set() -> None:
    ...
    assert ("CONTENT_PREP", "curate_segments") in _PHASE_STEP_TASK_TYPE_MATRIX
    assert _PHASE_STEP_TASK_TYPE_MATRIX[("CONTENT_PREP", "curate_segments")] == {
        "video_curation"
    }
    assert "video_curation" in _PHASE_TASK_TYPES["CONTENT_PREP"]
...
def test_video_curation_phase_step_combo_validates() -> None:
    """三元组合法校验：(CONTENT_PREP, curate_segments, video_curation) 通过；
    ...
    # 合法
    _validate_phase_step_task_type_combo(
        "CONTENT_PREP", "curate_segments", "video_curation"
    )
    # 非法：phase 错位
