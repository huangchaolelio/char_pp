...
3. **长视频支持**：将现有仅支持 ≤5 分钟的单次 Celery 任务改为分段串行处理，支持最长 60 分钟视频，并暴露实时进度查询接口。
...
**性能目标**:
- 音频转录：≤ 60 分钟视频在 10 分钟内完成转录（Whisper base 模型 CPU，约 4× 实时）
- 关键词定位：< 100ms（纯内存词表匹配）
- 长视频整体处理：60 分钟视频总处理时间 ≤ 40 分钟（目标，含音频+姿态）
...
**规模/范围**: 支持最长 60 分钟教学视频；音频转录条目数量级约 200-500 句/小时视频
...
- 修改 `expert_tech_point.py` 模型：新增 `source_type`、`transcript_ref`、`conflict_flag` 字段
...
| 成功标准 | 指标 | 基准建立方式 | 验证任务 |
|----------|------|-------------|---------|
| SC-001 | 音频→KB 贡献比 ≥ 50% | 标注 10 段含口头讲解视频，人工统计语音技术要点数 | T001 |
| SC-002 | 片段命中率提升 ≥ 30% | 同一视频对比纯视觉采样 vs. 音频定位，统计高峰帧比例 | T002 |
| SC-003 | 60 分钟视频完整处理，覆盖全部动作类型 | 使用标注完整教学视频验证覆盖率 | T003 |
| SC-004 | 进度更新延迟 ≤ 30s | 计时测量进度字段更新间隔 | T004 |
| SC-005 | 音频技术要点准确率 ≥ 80% | 20 条样本人工抽查对照原始转录文本 | T005 |
