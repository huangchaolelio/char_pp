...
- [x] T068 [P] [US5] 合约测试 `tests/contract/test_curation_stats.py`：覆盖 `contracts/curation_stats.md` 全部 6 条用例（**实施备注**：用例 5 "page_size=200" 文档原写 400 INVALID_PAGE_SIZE，实施时与项目其它路由（F-017 章程 v1.4.0）对齐，统一通过 Pydantic Query `le=100` 拦截，返 422 VALIDATION_FAILED；测试断言已对齐实际行为）

### US5 实施

- [x] T069 [P] [US5] 在 `src/api/schemas/curation.py` 中新增 `CurationStatsItem` Pydantic 模型（响应使用 `SuccessEnvelope[list[CurationStatsItem]]` + `meta` 分页信封，无需 `CurationStatsResponse` 包装类）
- [x] T070 [US5] 在 `src/services/curation/curation_service.py` 新增 `aggregate_curation_stats(group_by, filters, page, page_size) -> tuple[list[CurationStatsItemDTO], int]`：基于 `video_curation_jobs` + `coach_video_classifications` 两表 JOIN（**实施备注**：原计划三表含 `coaches` JOIN，实际 `coach_name` 字段直接落在 `coach_video_classifications`，无需再 JOIN `coaches`；文档原 § 3.4 表述按既有事实修正不再写）；分三阶段聚合：① 主聚合（video_count / avg_ratio / low_quality_count）② avg_validity_score（segment 粒度均值）③ with_overrides_video_count（仅 coach / tech_category 维度）
- [x] T071 [US5] 创建 `src/api/routers/curation_stats.py`：`GET /api/v1/curation-stats` 端点；`group_by` 用 `Literal` 强约束三枚举；`page_size` 用 `Query(le=100)` 拦截
- [x] T072 [US5] 在 `src/api/main.py` 注册 `curation_stats_router`（紧跟 `curation_jobs_router` 之后）
- [x] T073 [US5] 在 `aggregate_curation_stats` 实现中附 `low_sample=video_count < 5` 字段（在 DTO 拼接阶段判断），契约对齐

**检查点**: 全部 5 个用户故事独立功能化（US5 完成测试 6/6，单元覆盖在合约测试中）。
