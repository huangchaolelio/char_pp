...
> Feature-023 完成后首次端到端实测。目标：用一个真实视频走完
> COS 扫描 → preprocessing → KB DAG（6 步）→ KB 落库 整条链路，
> 验证 V2 字典口径下管线不裂、并捞出残留 bug。

### 实测目标

- 视频：`charhuang/tt_video/乒乓球合集【较新】/《知行合一》孙浩泓专业乒乓球全套教学课程120集/第58节正手起下旋.mp4`
- COS 扫描后分类结果：`action='高吊弧圈球'`，`category_l3='正手·进攻'`，`confidence=1.0`，`source=rule`
- 双 bypass 已开（`.env` 中 `KB_EXTRACTION_BYPASS_CURATION_GATE=true` + `KB_EXTRACTION_BYPASS_REVIEW_GATE=true`）—— 直接走应急通道，不依赖 curation/review 工作台

### 链路耗时

| 阶段 | 状态 | 耗时 | 关键产物 |
|---|---|---|---|
| `POST /tasks/preprocessing` → preprocess_video | ✅ success | **37 s** | 3 segments + 1 audio 上传 COS（`preprocessed/.../jobs/<job>/seg_000{0,1,2}.mp4`） |
| `POST /tasks/kb-extraction` → DAG 6 步合计 | ✅ success | **6 m 22 s** | `extraction_jobs.status=success`，`tech_knowledge_bases (action='高吊弧圈球', version=1, status=draft)` 新建，`coach_video_classifications.kb_extracted=True` 已回写 |
| └ download_video | ✅ | 1.9 s | curation_bypass=true，3 segments + audio 全下 |
| └ pose_analysis | ✅ | 123 s | YOLOv8 backend，1920×1080，13 437 keypoint frames，**`detected_segments=0`** |
| └ audio_transcription | ✅ | 251 s | Whisper-small/CPU，249 句 / 1 976 字，SNR 26.7 dB |
| └ visual_kb_extract | ✅ | 1.8 s | backend=`action_segmenter+tech_extractor`，产出 ≥3 条 visual KB items（如 `elbow_angle 68–117° (ideal 104°)`、`swing_trajectory 0.93–1.25`） |
| └ audio_kb_extract | ✅ | 15 ms | Venus qwen3.5-35b，12 parsed → 0 入库（全部 `dropped_reference_notes`，LLM 判为参考备注） |
| └ merge_kb | ✅ | 33 ms | `merged_items=4`、`conflict_items=0`、**`inserted_tech_points=0`** ⚠️ |

### 结论

✅ **链路全程跑通** —— Feature-013/014/016/021/022/023 在 V2 口径下端到端无裂缝：
任务通道 / DAG 编排 / 双 bypass / preprocessing 复用 / 视觉 + 音频抽取 / KB draft 创建 / classification 回写都按设计协作。

⚠️ **暴露 1 个真实代码 bug（Feature-023 重构遗留）**：

- 现象：`merge_kb` 的 `output_summary.inserted_tech_points=0`，但 `merged_items=4`；`expert_tech_points` 表整张为空（`SELECT COUNT(*)` = 0）
- 根因：[`src/models/expert_tech_point.py`](/data/charhuang/char_ai_coding/charhuang_pp_cn/src/models/expert_tech_point.py) 中 `ActionType` 枚举仍是 V1 的 21 个英文键（`forehand_loop_high` / `forehand_topspin_backspin` …），而 Feature-023 已把分类口径切到 V2 的 56 行中文 `tech_actions` 字典（`高吊弧圈球` 等）
- 触发链路：[`src/services/kb_extraction_pipeline/step_executors/merge_kb.py::_coerce_action_type`](/data/charhuang/char_ai_coding/charhuang_pp_cn/src/services/kb_extraction_pipeline/step_executors/merge_kb.py) 调 `ActionType('高吊弧圈球')` → `ValueError` → 返回 `None` →
  `_persist_merged_points` 中 silently `continue`，所有 4 条 merged 全被丢弃
- 日志特征：`merge_kb skip: dimension=<X> has no ActionType mapping (got action_type='高吊弧圈球', tech_category='高吊弧圈球')`
- 影响范围：所有 KB 提取任务 —— 从 Feature-023 上线（2026-05-31，迁移 0022）至 G2 修复之前，所有 `extraction_jobs.status=success` 的作业 `inserted_tech_points` 均为 0；线上 KB 表实际是空的
- 修复实施（**已在 G2 阶段落地**）：
  1. 新增迁移 [`0023_drop_action_type_enum.py`](/data/charhuang/char_ai_coding/charhuang_pp_cn/src/db/migrations/versions/0023_drop_action_type_enum.py)：DROP 旧 `action_type` 列 + DROP TYPE `action_type_enum`，ADD `action varchar(64) NOT NULL` + 复合 FK `(category_l1, l2, l3, action) → tech_actions(...)` + index；`submitted_action` String(50)→String(64)
  2. [`src/models/expert_tech_point.py`](/data/charhuang/char_ai_coding/charhuang_pp_cn/src/models/expert_tech_point.py) 删 `ActionType` 枚举类，字段 `action_type → action`（varchar + 复合 FK）
  3. [`src/services/kb_extraction_pipeline/step_executors/merge_kb.py`](/data/charhuang/char_ai_coding/charhuang_pp_cn/src/services/kb_extraction_pipeline/step_executors/merge_kb.py) `_coerce_action_type → _resolve_action`，改为查 `tech_actions` 字典（进程级缓存）
  4. 路由 / Schema / 其他 service 共 7 处 `action_type → action` 同步重命名
- 渐进验证（2026-06-02 G2 后用同一视频 `force=true` 重跑）： `merge_kb.inserted_tech_points=4`；`SELECT COUNT(*) FROM expert_tech_points WHERE kb_action='高吊弧圈球'` 返回 **4**。问题闭环。
> 次要观察（不阻断、不计 bug）：`pose_analysis.output_summary.detected_segments=0`
> 但 `keypoints_frame_count=13437`。说明 action segmenter 没有从 13k 帧里切出"动作片段"，
> visual_kb_extract 退到了基于全片 keypoints 的兜底统计。如果 `ActionType` bug 修复
> 后入库数据偏粗糙，这是下一个值得复查的位置。

### 复现要点

```bash
# 1) preprocessing 提交
curl -sS -X POST http://localhost:8080/api/v1/tasks/preprocessing \
  -H 'Content-Type: application/json' \
  -d '{"cos_object_key":"<COS_KEY>","force":false}'

# 2) KB 提取提交（前置 .env 双 bypass=true 已设）
curl -sS -X POST http://localhost:8080/api/v1/tasks/kb-extraction \
  -H 'Content-Type: application/json' \
  -d '{"cos_object_key":"<COS_KEY>","enable_audio_analysis":true,"audio_language":"zh","force":false}'

# 3) 验证产物
SELECT inserted_tech_points FROM pipeline_steps
  WHERE job_id=<extraction_job_id> AND step_type='merge_kb';
SELECT COUNT(*) FROM expert_tech_points WHERE kb_action='高吊弧圈球';
```

如 G2 未部署，第 3 步两个查询会分别返回 `0` 与 `0` —— 即可确认问题复现。实际 G2 部署后验证返回 **4** 与 **4**（渐进验证、问题闭环）。
