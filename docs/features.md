...
| `/api/v1/knowledge-base/{version}` | GET | ENDPOINT_RETIRED 哨兵（章程原则 IX）|
| `/api/v1/knowledge-base/{version}/approve` | POST | ENDPOINT_RETIRED 哨兵（章程原则 IX）|
...
- **原则 IX**：老单列 version 路径改为 `ENDPOINT_RETIRED` 哨兵保留；4 个新错误码同步 `src/api/errors.py` 三张映射表
