...
> Feature-017 合入后的 API 开发统一规范（对齐章程 v1.4.0 原则 IX）
>
> 权威参考：
...
> - `specs/017-api-standardization/contracts/retirement-ledger.md`
...
下线必须保留哨兵路由（方法+路径不变），处理器统一抛 `AppException(ENDPOINT_RETIRED, details={...})`，返回 HTTP 404 + 错误信封。

**操作步骤**：

1. 在 `src/api/routers/_retired.py::RETIREMENT_LEDGER` 追加条目（元组形式）
2. 在 `specs/<feature>/contracts/retirement-ledger.md` 同步追加台账行
3. 在 `tests/contract/test_retirement_contract.py` 新增对应 404 断言

**台账禁止删除条目**（审计需要），**禁止复用已下线路径**。
...
- 39 个错误码：`specs/017-api-standardization/contracts/error-codes.md`
- 7 个下线接口：`specs/017-api-standardization/contracts/retirement-ledger.md`
