...
# Feature 013 + 016 — 五队列多 Worker 架构 + 一个 Celery Beat，一队列一 Worker 物理隔离
# Worker 1：分类队列（并发=1）
...
# Celery Beat（全局唯一定时调度器，不带 worker/-Q 参数）
...
# ⚠️ 必须启动；不启动 ⇒ 周期任务全部停摆，通道槽位无法自动释放
# ⚠️ 集群内只能启 1 个 beat，多个会重复派发周期任务
setsid /opt/conda/envs/coaching/bin/celery -A src.workers.celery_app beat --loglevel=info --schedule=/tmp/celerybeat-schedule >> /tmp/celery_beat.log 2>&1 &
```
...
- `sweep_orphan_jobs`（`src.workers.housekeeping_task`）：回收 OOM / WorkerLostError 卡住的 running 任务（`analysis_tasks` / `extraction_jobs` / `pipeline_steps` / `video_preprocessing_jobs`），beat 驱动每 5 分钟一次，`default` 队列
